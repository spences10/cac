...
describe('convert_animal_age', () => {
	...
	test('returns the correct conversion from rabbit to horse years', () => {
		...
		const expected_horse_years =
			rabbit_years *
			(animal_age_units['rabbit'] / animal_age_units['horse'])
		expect(
	...
	test('returns null when given invalid units', () => {
		expect(convert_animal_age('invalid', 'dog', 10)).toEqual(null)
		expect(convert_animal_age('dog', 'invalid', 10)).toEqual(null)
	})

	test('returns the same value when converting from and to the same unit', () => {
		expect(convert_animal_age('human', 'human', 10)).toEqual(10)
		expect(convert_animal_age('cat', 'cat', 10)).toEqual(10)
	})
})
